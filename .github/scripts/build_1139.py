...
def main():
    ...
    try:
        ...
        new='''        if zone and plausible_zone_title(zone):\n            # Confirmed map title: publish through the normal updater, then\n            # mirror it to the overlay immediately. Avoid function-local global\n            # declarations because current_zone is already read above here.\n            update_zone(zone)\n            try:\n                live_overlay["zone"] = zone\n            except Exception:\n                pass\n\n        hover_item, hover_raw = read_hover_item(win)\n'''
        if old in block:
        ...
        with zipfile.ZipFile(nz) as z:
            ...
            for required in ('is_level_only_target_name','update_zone(zone)','live_overlay["zone"] = zone','ZONE SOURCE:'):
        ...
        M.write_text(json.dumps({'name':'EnB Droplist','version':V,'download_url':'https://soulbound.se/EnB/Download/EnB%20Droplist.zip','sha256':sha,'release_date':'2026-08-24','release_notes':'Confirmed map zone from OCR Status/Test is mirrored immediately to the F8 overlay. Fixes 1.1.39 build syntax error and keeps 1.1.38 target OCR fixes.'},indent=2)+'\n',encoding='utf-8')
