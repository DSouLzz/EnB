...
R=Path.cwd(); Z=R/'release'/'EnB Droplist.zip'; M=R/'release'/'version.json'; V='1.1.36'; BASE='8ab688349d30715d3c009e71a6c068180fdac6ac'
...
def ok():
    try:
        ...
        print('release already valid',V,sha,Z.stat().st_size);return True
    except Exception:return False

def main():
    if ok():return
    w=Path(tempfile.mkdtemp(prefix='enb136_verified_'))
    try:
        base=w/'base.zip'
        base.write_bytes(subprocess.check_output(['git','show',f'{BASE}:release/EnB Droplist.zip']))
        with zipfile.ZipFile(base) as z:
            if z.testzip():raise SystemExit('bad base zip')
        ...
        old2='        if (map_open or probe_support >= 2) and (fallback_zone or probe_zone) and plausible_zone_title(fallback_zone):\n            chosen = fallback_zone if fallback_zone and plausible_zone_title(fallback_zone) else probe_zone\n'
        new='        if (map_open or probe_support >= 3) and ((fallback_zone and plausible_zone_title(fallback_zone)) or (probe_zone and probe_support >= 3 and plausible_zone_title(probe_zone))):\n            chosen = fallback_zone if fallback_zone and plausible_zone_title(fallback_zone) else probe_zone\n'
        if old in s:s=s.replace(old,new,1)
        elif old2 in s:s=s.replace(old2,new,1)
        else:raise SystemExit('F8 zone condition missing')
        ...
        c=a/'config.json';d=json.loads(c.read_text(encoding='utf-8'));d.setdefault('behavior',{});d['behavior']['map_title_probe_min_support']=3;d['behavior']['ocr_status_worker']=True;c.write_text(json.dumps(d,indent=2),encoding='utf-8')
